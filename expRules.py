...
def generate_joint_rules(joint_positions):
    ...
    if (STRAIGHT>abs(180-calculate_angle(joint_positions['left_hip'],joint_positions['left_knee'],joint_positions['left_foot']))):
        rules['left_leg_extension'] = 'extended'
    if (STRAIGHT>abs(180-calculate_angle(joint_positions['right_hip'],joint_positions['right_knee'],joint_positions['right_foot']))):
    ...
    # One armed
    if CLOSE>calculate_distance(joint_positions['left_hand'],joint_positions['left_shoulder']):
        rules['left_arm_position'] = 'close_to_chest'
    if CLOSE>calculate_distance(joint_positions['right_hand'],joint_positions['right_shoulder']):
        rules['right_arm_position'] = 'close_to_chest'
    if (rules['right_arm_position'] == 'close_to_chest') and (rules['left_arm_position'] = 'close_to_chest'):
        rules['arm_position'] == 'close_to_chest'
